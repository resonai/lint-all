...
REQUIRED_PIP = ('cpplint,mypy,pyenchant,pylint,types-mock,types-protobuf,'
...
def map_line_numbers(file_name: str, repo: Repo) -> tuple[list[int], list[int]]:
  ...
  while i < len(lines):
    ...
    if match:
      ...
      p_pair = [
          int(re.search(r"^.*,", c).group()[1:-1])  # type: ignore
          for c in match.group()[3:-3].split()
      ]
      i += 1
  ...
  old_line_count = len(
      repo.git.show(f'{ref_branch}:{file_name}').split('\n')) if git_exists(
          repo, ref_branch, file_name) else 0
  new_line_count = 0
...
def extract_issues_from_linter_output(cmd: list[str], env: dict[str, str],
                                      file_name: str,
                                      use_stderr: bool) -> list[str]:
  """
  Run linter command and extract stdout or stderr lines that begin with the file
  name.
  """
  output = run(
      cmd + [file_name], stdout=PIPE, stderr=PIPE, check=False,
      env=env) if env else run(
          cmd + [file_name], stdout=PIPE, stderr=PIPE, check=False)
  output_txt = output.stderr if use_stderr else output.stdout
  return [
      l for l in output_txt.decode('utf-8').splitlines()
      if l.startswith(file_name)
  ]


def run_linter_ignore_old_issues(
    cmd: list[str], env: dict[str, str], file_name: str, ref_dir: str,
    new_to_old: list[int], old_to_new: list[int],
    use_stderr: bool) -> tuple[list[str], list[str]]:
  """
  ...
  issues_ref = extract_issues_from_linter_output(cmd=cmd,
                                                 env=env,
                                                 file_name=ref_file_name,
                                                 use_stderr=use_stderr)
  issues_new = extract_issues_from_linter_output(cmd=cmd,
                                                 env=env,
                                                 file_name=file_name,
                                                 use_stderr=use_stderr)
  ...
  def is_issue_from_a_in_b(a_issue, a_file_name, b_file_name, b_issues,
                           a_to_b_line_mapping):
  ...
  for i in range(2):
    for issue in old_new_issues[i]:
      j = (i + 1) % 2
      if is_issue_from_a_in_b(issue, old_new_file[i], old_new_file[j],
                              old_new_issues[j], old_new_mappings[i]):
        issues[i].append(issue)
  return issues


def lint_file(file_name: str, ref_dir: str, new_to_old: list[int],
              old_to_new: list[int],
              linter: Linter) -> tuple[list[str], list[str]]:
  """
  ...
  if not old_to_new or GLOBAL_FLAGS.report_old_issues:
    issues = [], extract_issues_from_linter_output(linter.cmd, {},
                                                   file_name,
                                                   use_stderr=linter.use_stderr)
  else:
    issues = run_linter_ignore_old_issues(linter.cmd, {},
                                          file_name,
                                          ref_dir,
                                          new_to_old,
                                          old_to_new,
...
def filter_types_and_folders(linters: list[Linter],
...
def check_installations():
  installed_pkgs = [x.key for x in list(pkg_resources.working_set)]
  missing_pkgs = [
      req for req in REQUIRED_PIP.split(',') if req not in installed_pkgs
  ]
  if missing_pkgs:
    missing_pkgs_str = ' '.join(missing_pkgs)
    print(f'{RED}Missing python package installations for {missing_pkgs_str}'
          f'{ENDC}.\nRun:\npip install {missing_pkgs_str}')
    sys.exit(1)


def main(linters: list[Linter]):
  """
  This is intentionally not a 'ybt code' in the sense that it doesn't use
  ywz flags or other mechanisms. The reason is to accommodate Mac users, since
  running git (and other file system operations) inside docker on Mac is
  incredibly slow. The apparent reason is that the code running inside the
  docker is compiled for x86 instruction set, and therefore has more translation
  layers to go through, which slows is down by a factor of 10-30.

  When we start generating mac-compatible images, we can go back to using this
  inside a ybt target.

  Note: non-Mac users can still call this script from a ./ybt_bin/... path and
  avoid having to deal with package installations.
  """
  check_installations()
  repo = Repo()
  tmp_dir = ""
  try:
    ...
    if GLOBAL_FLAGS.check_all_files:
      ...
      changed_files = [
          item.a_path for item in repo.index.diff(GLOBAL_FLAGS.ref_branch)
      ]
      ...
      changed_files = [
          x for x in changed_files
          if git_exists(repo, repo.head.object.hexsha, x)
      ]
  ...
  try:
    ...
    print(f'{WHITE}Running {len(linters)} linters '
    ...
    for i, fname in enumerate(changed_files):
      print(
          f'{YELLOW}Analyzing file {fname} ({i + 1}/{len(changed_files)}):{ENDC}'
      )
      issues: tuple[list[str], list[str]] = ([], [])
      ...
      for linter in linters:
        if fname.endswith(tuple(linter.extensions)):
          extend_both(issues,
                      lint_file(fname, tmp_dir, new_to_old, old_to_new, linter))
...
def add_bool_flag(cli_parser: argparse.ArgumentParser, flag_name: str,
                  default_val: bool, help_str: str):
  feature_parser = cli_parser.add_mutually_exclusive_group(required=False)
  feature_parser.add_argument(f'--{flag_name}',
                              dest=f'{flag_name}',
                              action='store_true',
                              help=help_str)
  feature_parser.add_argument(f'--no{flag_name}',
                              dest=f'{flag_name}',
                              action='store_false')
...
if __name__ == '__main__':
  ...
  parser.add_argument('--linters_config',
                      type=str,
                      default='scripts/lint/all_linters.yaml',
  ...
  for linter in all_linters:
    add_bool_flag(cli_parser=parser,
                  flag_name=linter.name,
                  default_val=True,
                  help_str=f'Run {linter.name}')
  parser.add_argument('--ref_branch',
                      type=str,
                      default='origin/master',
                      help='Reference branch against which the current branch '
                      'is compared')
  add_bool_flag(cli_parser=parser,
                flag_name='check_all_files',
                default_val=False,
                help_str='Run with all files. If false, runs only on diff '
                'from ref_branch')
  add_bool_flag(cli_parser=parser,
                flag_name='report_old_issues',
                default_val=False,
                help_str='Also include old issues in the report (if not set '
                '- ignores old issues).')
  add_bool_flag(cli_parser=parser,
  ...
  used_linters = [
      linter for linter in all_linters if vars(GLOBAL_FLAGS)[linter.name]
  ]
  main(used_linters)
